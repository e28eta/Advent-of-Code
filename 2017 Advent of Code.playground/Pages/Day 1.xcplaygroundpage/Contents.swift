...
/*:
 ...
 When your eyes can focus again, everything seems a lot more pixelated than before. She must have sent you inside the computer! You check the system clock: **25 milliseconds** until midnight. With that much time, you should be able to collect all _fifty stars_ by December 25th.
...
func captcha(_ string: String, offset: Int) -> Int {
    let numbers = string.flatMap { Int("\($0)") }
    let seq = zip(numbers, RotatedRandomAccess(wrapped: numbers, offset: offset))
    return seq.filter { $0.0 == $0.1 }.reduce(0) { $0 + $1.0 }
}

func captchaNextDigit(_ string: String) -> Int {
    return captcha(string, offset: 1)
}

verify(testData, captchaNextDigit)

let input = try readResourceFile("input.txt")
let part1 = captchaNextDigit(input)
assertEqual(part1, 1343)
...
func captchaHalfway(_ string: String) -> Int {
    // use offset == length/2 for part two
    return captcha(string, offset: string.count / 2)
}

verify(testData2, captchaHalfway)

let part2 = captchaHalfway(input)
assertEqual(part2, 1274)

// Interestingly, we only need to look at half of the input, since matches are reflexive

func captchaHalfHalfway(_ string: String) -> Int {
    let numbers = string.flatMap { Int("\($0)") }
    let offset = numbers.count / 2
    let seq = zip(numbers[..<offset], numbers[offset...])
    return seq.filter { $0.0 == $0.1 }.reduce(0) { $0 + $1.0 + $1.1 }
}

verify(testData2, captchaHalfHalfway)
assertEqual(part2, captchaHalfHalfway(input))
