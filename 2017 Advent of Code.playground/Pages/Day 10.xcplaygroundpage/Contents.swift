...
verify(testData, {
    var hash = KnotHash(UInt8($0.0))
    return hash.apply($0.1).checksum
})

let input = "83,0,193,1,254,237,187,40,88,27,2,255,149,29,42,100"
let part1Input = input.split(separator: ",").flatMap { Int($0) }

var hash = KnotHash(255)
hash.apply(part1Input)
...
verify(testData2, { KnotHash.hash($0) })
assertEqual(KnotHash.hash(input), "d9a7de4a809c56bf3a9465cb84392c8e")
