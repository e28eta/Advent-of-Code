...
verify(testData, { testInput in
    let (size, lengths) = testInput
    return lengths.reduce(into: KnotHash(size), { $0.apply($1) }).checksum
})

let input = "83,0,193,1,254,237,187,40,88,27,2,255,149,29,42,100".split(separator: ",").flatMap { Int($0) }

var hash = input.reduce(into: KnotHash(256)) { $0.apply($1) }
assertEqual(hash.checksum, 20056)
