...
// open grid
// does it matter? given position of head, tail, possible to calculate next positions
// keep set of "positions tail visited" and then just count?
// hypothesis, probably change rules of movement for p2, or maybe need to keep frequency counts?
...
verify([
    ...
    let final = instructions.reduce(into: Rope()) { rope, instruction in rope.apply(instruction) }
    return final.tailPositionCount()
}
