...
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw
""".lines().map(Rucksack.init)

let input = try readResourceFile("input.txt").lines().map(Rucksack.init)

verify([
    (testInput, 157),
    (input, 7793),
], { input in
    input.reduce(0) { $0 + Int($1.duplicateItemPriority) }
})
...
extension Sequence {
    func everyThree() -> some Sequence<(Element, Element, Element)> {
        return sequence(state: makeIterator()) { iter in
            guard let one = iter.next(), let two = iter.next(), let three = iter.next() else { return nil }
...
verify([
    ...
    input.map(\.both)
        .everyThree()
        .compactMap {
            $0.0.intersection($0.1).intersection($0.2).first?.priority
        }
        .reduce(0) { $0 + Int($1) }
}
