...
verify([
    ...
    s.lines()
        .compactMap(Blueprint.init)
        .reduce(0) { sum, blueprint in
            sum + (blueprint.number * blueprint.geodeCount(after: 24))
        }
}
...
verify([
    (testInput, 3472), // 2 hours
    (input, 4212), // 2.5 mins
], measure: true) { input in
    input.lines()
        .compactMap(Blueprint.init)
        .prefix(3)
        .reduce(1) { product, blueprint in
            product * blueprint.geodeCount(after: 32)
        }
}
