...
func part1(_ memory: String) throws -> Int? {
    return try (0..<5).permutations().map { phases in
        return try phases.reduce(0) { (previous, phase) in
            var computer = IntcodeComputer(memory)
            computer.input = [phase, previous].makeIterator()
            try computer.run()
