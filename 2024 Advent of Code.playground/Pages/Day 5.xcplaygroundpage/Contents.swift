...
extension PageQueue {
    func part2() -> Int {
        ...
        for update in incorrectlyOrderedUpdates {
            // idea: set of pages that need to be printed
            // find which page could be printed without violating a rule,
            // by subtracting all of the successor page numbers for the pages left to be printed
            // should be one or more pages (probably one) at each step
            // honestly could just go halfway through and quit

            var printedOrder = [Int]()
            var remainingToBePrinted = Set(update)

            while printedOrder.count <= (update.count / 2) {
                var nextPageCandidates = remainingToBePrinted
                for page in remainingToBePrinted {
                    nextPageCandidates.subtract(rules[page, default: Set()])
                }

                guard nextPageCandidates.count > 0, let nextPage = nextPageCandidates.first else {
                    fatalError("No candidates left")
                }
                printedOrder.append(nextPage)
                remainingToBePrinted.remove(nextPage)
            }

            sum += printedOrder[update.count / 2]
        }
