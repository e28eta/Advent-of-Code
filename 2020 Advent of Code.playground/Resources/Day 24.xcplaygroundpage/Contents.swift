...
let exampleGrid = HexGrid(input: exampleInput, defaultValue: TileState.white)
let inputGrid = HexGrid(input: input, defaultValue: TileState.white)

extension HexGrid where Element == TileState {
    public var blackCount: Int { visitedStates { $0 == .black }.count }
}
...
verify([
    ...
    return currentState.blackCount
}
