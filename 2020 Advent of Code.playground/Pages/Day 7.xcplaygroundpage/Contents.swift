...
class BagRules {
    let bags: [Color: Bag]
    var requiredContentCount: [Color: Int?] = [:]

    func contentCount(for color: Color) -> Int {
        switch requiredContentCount[color] {
        case .some(.some(let count)):
            return count
        case .some(.none):
            fatalError("cycle detected? \(color)")
        case .none:
            ...
            requiredContentCount[color] = count
            return count
        }
    }

    init(_ rules: [Bag]) {
        bags = Dictionary(grouping: rules, by: \.color).mapValues {
            guard $0.count == 1 else { fatalError(">1 rule for \($0.first!.color)")}
            return $0.first!
        }
    }
}
...
verify([
    ...
    BagRules(bags).contentCount(for: "shiny gold")
}
