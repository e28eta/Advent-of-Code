...
struct Combat {
    ...
    func play() -> Int {
        while !players.0.deck.isEmpty && !players.1.deck.isEmpty {
            let cards = (players.0.deck.removeFirst(), players.1.deck.removeFirst())

            if cards.0 > cards.1 {
                players.0.deck.append(cards.0)
                players.0.deck.append(cards.1)
            } else if cards.1 > cards.0 {
                players.1.deck.append(cards.1)
                players.1.deck.append(cards.0)
            } else {
                fatalError("tied cards: \(cards.0) & \(cards.1)")
            }
        }

        let winningDeck = players.0.deck.isEmpty ? players.1.deck : players.0.deck
...
verify([
    (exampleInput, 291),
    (input, 30891), // super slow, but it _does_ finish...
]) {
    RecursiveCombat($0)?.play()
}
