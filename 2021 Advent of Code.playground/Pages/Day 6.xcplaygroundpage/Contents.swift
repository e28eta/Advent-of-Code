...
public struct LanternfishSchool {
    let originalCounts: [Int]

    public init(_ string: String) {
        var counts = Array(repeating: 0, count: 9)

        for timer in string.components(separatedBy: ",").compactMap(Int.init) {
            guard counts.indices.contains(timer) else {
                fatalError()
            }
            counts[timer] += 1
        }
    ...
    public func part1() -> Int {
        ...
        for _ in (0 ..< 80) {
            ...
            counts[adultDay] += counts[newbieDay]
        }
