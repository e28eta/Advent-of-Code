...
assert(SnailfishNumber("[[[[9,1],2],3],4]").canExplode(depth: 1) == false)

verify(explodeTests) {
    var num = SnailfishNumber($0)
    num.explode(depth: 1)
...
let reduceOnceTests = [
    ...
    // one non-reducible case:
    ("[[[[0,7],4],[[7,8],[6,0]]],[8,1]]", "[[[[0,7],4],[[7,8],[6,0]]],[8,1]]"),
]

verify(reduceOnceTests) {
    var num = SnailfishNumber($0)
    num.reduceOnce()
    return num.description
}
...
verify(additionTests) {
    (SnailfishNumber($0.0) + SnailfishNumber($0.1)).description
}
...
verify(magnitudeTests) {
    SnailfishNumber($0).magnitude
}

func part1(_ string: String) -> Int {
    let numbers = string.lines().map(SnailfishNumber.init)

    return numbers.dropFirst()
        .reduce(numbers.first!) { (left, right) in
            return (left + right).reduced()
        }
        .magnitude
}

verify(chain(testCases, [(input, 4072)]), part1)
...
func part2(_ string: String) -> Int {
    ...
    return numbers
        ...
        .map { nums in
            (nums[0] + nums[1]).reduced().magnitude
        }
        .max() ?? -1
}
