...
let test = TargetArea(xRange: (20 ... 30),
                      yRange: (-10 ... -5))
let actual = TargetArea(xRange: (195 ... 238),
                        yRange: (-93 ... -67))
...
verify([
    ...
    validYVelocities(landingInside: $0)
        .map(\.peak)
        .max() ?? -1
}
...
verify([
    ...
    // just calculate all possible x / y velocities that
    // have values in target area
    let xVelocityCandidates = validXVelocities(landingInside: targetArea.xRange)
    let yVelocityCandidates = validYVelocities(landingInside: targetArea.yRange)

    // and then check every pair of velocities
    return product(xVelocityCandidates,
                   yVelocityCandidates)
    .filter { (Vx, Vy) in
        // for an x,y pair inside the target area
        zip(Vx.allXValues(), Vy.yValues)
            .contains { (x, y) in
                targetArea.xRange.contains(x)
                && targetArea.yRange.contains(y)
            }
    }
    // and count them
    .count
